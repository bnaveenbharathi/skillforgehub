...
const QuickLinks = () => {
  const links = [
    ...
    {
      title: "Certificates",
      description: "Verify blockchain certificates",
      icon: Shield,
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-4">
      {links.map((link, index) => (
        ...
        >
          <Link
            to={link.href}
            className="card-feature text-left cursor-pointer group"
          >
            <div className="flex items-center space-x-4">
              <div
                className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${link.color} flex items-center justify-center group-hover:scale-110 transition-transform duration-200`}
              >
